...
## 11. Corrections Made During Implementation (2026-09-23)

- **Wrong FK type:** `docs/domain-model.md` §3.2 listed `LossEvents.CauseOfLossCode` only as "(FK)" without specifying the target column, and the initial entity implementation guessed a `Guid CauseOfLossCodeId` FK to `CauseOfLossCodes.CauseOfLossCodeId`. The FRS source (§9.2) is explicit: `CauseOfLossCode | FK to CauseOfLossCodes.Code (NVARCHAR(50), NOT NULL)` — a string FK to the `Code` alternate key, not the surrogate GUID PK. Corrected in the entity, EF configuration (added `HasAlternateKey(c => c.Code)` on `CauseOfLossCodes`, replacing an incorrectly org-scoped composite unique index), and `docs/domain-model.md` §3.2/§3.9.
